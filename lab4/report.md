...
Итог: оптимальным для первого изображения является порог 50, так как при пороге 40 между строчками виден шум, а при пороге 60 теряются большие буквы (например на пятой строке)
...
Итог: оптимальным для второго изображения является порог 40, так как при пороге 50 откусываются части букв, а при пороге 30 слишком много шума и контур слишком жирный, из-за чего буквы смазываются или превращаются в белые пятна
...
Итог: для третьего изображения вероятно не подходит данный алгоритм построения модуля градиента, так как при пороге хоть и шум выражен простым "соль и перец", но стираются большие части букв, а при пороге 20 буквы становятся не отличимы от шума
...
Итог: оптимальным для четвертого изображения является порог 30, так как при меньшем пороге слишком много шумов, а при большем пороге перестают различаться некоторые буквы
...
Итог: для пятого изображения вероятно не подходит данный алгоритм построения модуля градиента, так как при пороге 40 хоть и хорошо различается черный текст и шумом является простой "соль и перец", но красный текст почти стерся, при пороге 30 красный текст становится более различимым, но шума возможно многовато, при пороге 20 слишком много шумов
